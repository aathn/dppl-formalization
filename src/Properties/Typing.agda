...
≤ᶜ⇒⊙ : c ≤ᶜ T → c ⊙ T ≡ T
≤ᶜ⇒⊙ {c = c} {T = treal c′} H≤ = ap treal $ i≤j⇒i⊔′j≡j H≤
≤ᶜ⇒⊙ {T = T₁ ⇒[ _ ] T₂} (H≤₁ , H≤₂) = ap₂ (_⇒[ _ ]_) (≤ᶜ⇒⊙ H≤₁) (≤ᶜ⇒⊙ H≤₂)
≤ᶜ⇒⊙ {T = ttup n Ts} H≤ = ap (ttup n) $ funext λ i → ≤ᶜ⇒⊙ (H≤ i)
≤ᶜ⇒⊙ {T = tdist T} H≤ = refl

-- ≤ᶜ-trans : c ≤′ c′ → c′ ≤ᶜ T → c ≤ᶜ T
-- ≤ᶜ-trans {T = treal c″} H≤ H≤′ = ≤′-totalOrder.trans H≤ H≤′
-- ≤ᶜ-trans {T = T Type.⇒[ _ ] T₁} H≤ H≤′ = ≤ᶜ-trans H≤ (H≤′ .π₁) , ≤ᶜ-trans H≤ (H≤′ .π₂)
-- ≤ᶜ-trans {T = ttup n Ts} H≤ H≤′ i = ≤ᶜ-trans H≤ (H≤′ i)
-- ≤ᶜ-trans {T = tdist T} H≤ H≤′ = tt
...
sub-⊙-mono (sreal H≤) = sreal $ ⊔′.⊔-monoʳ-≤ _ H≤
sub-⊙-mono (stup Hsubs) = stup $ sub-⊙-mono ∘ Hsubs
sub-⊙-mono (sarr Hsub Hsub₁ H≤) = sarr (sub-⊙-mono Hsub) (sub-⊙-mono Hsub₁) H≤
sub-⊙-mono (sdist Hsub) = sdist Hsub

...
module _ (S : Set){{_  : Unfinite S}} where
  ...
  ----------------------------------------------------------------------
  -- Name swapping function [Equation (47)]
  ----------------------------------------------------------------------
  infix 6 _≀_
  _≀_ : S → S → S → S
  (a ≀ b) c =
    if does (a ≐ c) then b else
    if does (b ≐ c) then a else c

  ----------------------------------------------------------------------
  -- As mentioned in the proof of Proposition 3.7, a xioms TS₁-TS₄ imply
  -- equation (6.6) from [AM Pitts, Nominal Sets].
  --
  -- (Straightforward proof that conversely (6.6) implies TS₃ and TS₄ is
  -- omitted.)
  ----------------------------------------------------------------------
  module _ {X : Set}{{_ : TS X}} where
    τ-symm :
      (a b : S)
      (x : X)
      → ---------------
    ...
    τ-equivar :
      ...
      (x : X)
      → -------------------------------------------------
      τ a b (τ c d x) ≡ τ ((a ≀ b)c) ((a ≀ b)d) (τ a b x)
    τ-equivar a _ c _ _ with a ≐ c
    τ-equivar a _ _ d _ | equ with a ≐ d
    τ-equivar a b _ _ x | equ | equ =
      proof
        τ a b (τ a a x)
      ≡[ ap (τ a b) (TS₁ _ _) ]
        τ a b x
      [ TS₁ _ _ ]≡
        τ b b (τ a b x)
      qed
    τ-equivar _ b _ d _ | equ | neq _ with b ≐ d
    τ-equivar a b c d x | equ | neq f | equ =
      proof
        τ a d (τ a d x)
      ≡[ TS₂ _ _ _ ]
      x
      [ TS₂' _ _ _ ]≡
        τ d a (τ a d x)
      qed
    τ-equivar a b _ d x | equ | neq f | neq g =
      proof
        τ a b (τ a d x)
      ≡[ ap (τ a b) (τ-symm _ _ _) ]
        τ a b (τ d a x)
      ≡[ TS₄ _ _ _ _ {{ ¬≡→≠ g}} {{symm≠ a d (¬≡→≠ f)}} ]
        τ d b (τ a b x)
      ≡[ τ-symm _ _ _ ]
        τ b d (τ a b x)
      qed
    τ-equivar _ b c _ _ | neq _ with b ≐ c
    τ-equivar a _ _ d _ | neq _ | equ with a ≐ d
    τ-equivar a _ c _ _ | neq _ | equ | equ =
      ap (τ a c) (τ-symm _ _ _)
    τ-equivar _ _ c d _ | neq _ | equ | neq _ with c ≐ d
    τ-equivar a _ _ d x | neq _ | equ | neq _ | equ =
      proof
        τ a d (τ d d x)
      ≡[ ap (τ a d) (TS₁ _ _) ]
        τ a d x
      [ TS₁ _ _ ]≡
        τ a a (τ a d x)
      qed
    τ-equivar a _ c d x | neq _ | equ | neq g | neq h =
      proof
        τ a c (τ c d x)
      ≡[ ap (τ a c) (τ-symm _ _ _) ]
        τ a c (τ d c x)
      ≡[ τ-symm _ _ _ ]
        τ c a (τ d c x)
      ≡[ TS₄ _ _ _ _ {{¬≡→≠ g}} {{symm≠ c d (¬≡→≠ h)}} ]
        τ d a (τ c a x)
      ≡[ ap (τ d a) (τ-symm _ _ _) ]
        τ d a (τ a c x)
      ≡[ τ-symm _ _ _ ]
        τ a d (τ a c x)
      qed
    τ-equivar a _ _ d _ | neq _ | neq _ with a ≐ d
    τ-equivar a b c _ x | neq f | neq g | equ =
      TS₄ _ _ _ _ {{¬≡→≠ g}} {{symm≠ a c (¬≡→≠ f)}}
    τ-equivar _ b _ d _ | neq _ | neq _ | neq _ with b ≐ d
    τ-equivar a _ c d x | neq f | neq g | neq _ | equ =
      proof
        τ a d (τ c d x)
      ≡[ τ-symm _ _ _ ]
        τ d a (τ c d x)
      ≡[ TS₄ _ _ _ _ {{¬≡→≠ f}} {{symm≠ d c (¬≡→≠ g)}} ]
        τ c a (τ d a x)
      ≡[ ap (τ c a) (τ-symm _ _ _) ]
        τ c a (τ a d x)
      qed
    τ-equivar a b c d x | neq f | neq g | neq h | neq k =
      TS₃ _ _ _ _ _ {{¬≡→≠ g}}{{symm≠ a c (¬≡→≠ f)}}
      {{¬≡→≠ h}}{{symm≠ b d (¬≡→≠ k)}}
  ...
  module _ {X : Set}{{_ : fsRenset X}} where
    ...
    ... | neq f =
      ...
      proof
        ρ b c (ρ a b (ρ c a x))
      [ ap (ρ b c) (♯≡ _ d b {{♯ρ _ b a d {{♯ρ x a c d}}}}) ]≡
  ...
  fsRenset→TS {X} = record
    ...
    ... | neq f =
      ...
      proof
        σ a b (σ b a x)
      ≡[ σfresh a b c' (σ b a x) ]
        ρ b c' (ρ a b (ρ c' a (σ b a x)))
      ≡[]
        ρ b c' (ρ a b (ρ c' a (ρ a c (ρ b a (ρ c b x)))))
      ≡[ ap (ρ b c' ∘ ρ a b) (RN₃ _ _ _ _) ]
        ρ b c' (ρ a b (ρ c' c (ρ c' a (ρ b a (ρ c b x)))))
      ≡[ ap (ρ b c') (RN₄ _ _ _ _ _) ]
        ρ b c' (ρ c' c (ρ a b (ρ c' a (ρ b a (ρ c b x)))))
      ≡[ ap (ρ b c' ∘ ρ c' c ∘ ρ a b) (RN₂ _ _ _ _) ]
        ρ b c' (ρ c' c (ρ a b (ρ b a (ρ c b x))))
      ≡[ ap (ρ b c' ∘ ρ c' c) (RN₃ _ _ _ _)  ]
        ρ b c' (ρ c' c (ρ a a (ρ a b (ρ c b x))))
      ≡[ ap (ρ b c' ∘ ρ c' c) (RN₁ _ _) ]
        ρ b c' (ρ c' c (ρ a b (ρ c b x)))
      ≡[ ap (ρ b c' ∘ ρ c' c) (RN₂ _ _ _ _) ]
        ρ b c' (ρ c' c (ρ c b x))
      ≡[ ap (ρ b c') (RN₃ _ _ _ _) ]
        ρ b c' (ρ c' b (ρ c' c x))
    ...
    f₃ a b c d x =
      ...
      [ ap (ρ d e' ∘ ρ c d ∘ ρ e' c) (σfresh a b e x) ]≡
        ρ d e' (ρ c d (ρ e' c (σ a b x)))
      [ σfresh c d e' (σ a b x) ]≡
        σ c d (σ a b x)
      qed
...
lns→TSunique X ts p q = TSext ℕ𝔸 ts (lns→TS X) f g
  ...
  g (ι₂ a) (ι₂ b) x =
    ...
    proof
      ε₁ (ι₂ a) (ι₂ b) x
    [ ap (ε₁ (ι₂ a) (ι₂ b)) (≻3 i≻x ≤refl) ]≡
      ε₁ (ι₂ a) (ι₂ b) ((i ~> a)x)
    [ ap (ε₁ (ι₂ a) (ι₂ b)) (p i a _) ]≡
      ε₁ (ι₂ a) (ι₂ b) ((ε₁ (ι₂ a) (ι₁ i) x))
    [ TS₇{{_}}{{ts}} (ι₂ b) (ι₁ i) (ι₂ a) _  ]≡
  ...
  g (ι₁ i) (ι₁ j) x =
    ...
    [ ap (ε₁ (ι₁ i) (ι₁ j)) (#2 it) ]≡
      ε₁ (ι₁ i) (ι₁ j) ((i <~ a)x)
    [ ap (ε₁ (ι₁ i) (ι₁ j)) (q i a _) ]≡
      ε₁ (ι₁ i) (ι₁ j) (ε₁ (ι₁ i) (ι₂ a) x)
    [ TS₇{{_}}{{ts}} (ι₁ j) (ι₂ a) (ι₁ i) _ ]≡

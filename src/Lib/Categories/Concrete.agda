...
module _ {o ℓ e i p : Level} {𝒞 : CCat o ℓ e} (S : CSite 𝒞 i p) (o′ e′ : Level) where
  ...
  -- semantics: finite completeness, cartesian closure, as well as
  -- coproducts.
  ...
  CSheaves-Cartesian : Cartesian CSheaves
...
module _ {o i p : Level} {𝒞 : CCat o o o} (S : CSite 𝒞 i p) where
  open CCat 𝒞
  open CSheaf
  ...
  private
    ...
    module Cart = Cartesian (CSheaves-Cartesian S o o)
    module Prod = BinaryProducts Cart.products

  CSheaves-CartesianClosed : CartesianClosed (CSheaves S o o)
  CSheaves-CartesianClosed = record
    { cartesian = CSheaves-Cartesian S o o
    ; exp = λ {A} {B} →
      let module A = CSheaf A
          module B = CSheaf B
          module PreExp = Exponential (Presheaves-CartesianClosed.exp {A = A.Psh} {B = B.Psh})
      in record
      { B^A = record
        { Psh = PreExp.B^A
        ; is-sheaf = λ {U} {R} HR p matching → {!!}
